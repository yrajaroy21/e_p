...
if(isset($Action) && $Action == "login"){
    ...
    try {
        ...
        $VenueArray = array();
        $VenueArray["name"]         = $name;
        $VenueArray["location"]     = $location;
        $VenueArray["floor"]        = $floor;
        $VenueArray["capacity"]     = $capacity;
        $VenueArray["area"]         = $area;
        $VenueArray["image"]        = $image_file_path;
        $VenueArray["featured"]     = 'NO';
        $VenueArray["status"]       = 'Available';
    ...
    try {
        ...
        $ScheduleEventArray["event_venue"]                    = $getChooseVenue;
    ...
    try {
        ...
        if (mysqli_num_rows($Results) > 0) 
        {
            while($record = mysqli_fetch_assoc($Results)) 
            {
                ...
                $data["event_venue"]       = $record["event_venue"];
